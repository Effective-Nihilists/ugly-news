...
export default function PodcastPage(): React.ReactElement {
  ...
  const activeSegIdx = React.useMemo(() => {
    if (!podcast) return -1;
    return podcast.segments.findIndex(
      (s) => posMs >= s.startTimeMs && posMs < s.endTimeMs,
    );
  }, [podcast, posMs]);
  ...
  const dateStr = podcast
    ? new Date(`${podcast.date}T00:00:00Z`)
    ...
    >
      ...
      >
        ...
        >
          ...
          <a
            href="/archive?tab=podcasts"
            onClick={navClick(() => {
              router.push('archive', { tab: 'podcasts' });
            })}
            className="pp-back"
            data-id="all-episodes"
          >
            All episodes →
          </a>
        </div>
        ...
        {status === 'none' && (
          ...
          >
            ...
            >
              Today’s broadcast hasn’t been recorded yet. The hosts run every
              morning — check back shortly, or read the front page in the
              meantime.
            </p>
            <a
              href="/#front"
        ...
        {status === 'recording' && (
          ...
          >
            ...
            >
              The hosts are in the booth scripting and voicing today’s episode.
              This page will start playing automatically the moment it’s ready.
            </p>
          </div>
        )}
        ...
        {status === 'failed' && (
          ...
          >
            Today’s recording didn’t make it to air. We’ll try again on the next
            cycle.
          </p>
        )}
        ...
        {status === 'ready' && podcast && (
          <>
            ...
            {avatars.h1 && avatars.h2 ? (
              <PodcastStage
                podcast={podcast}
                host1AvatarUrl={avatars.h1}
                host2AvatarUrl={avatars.h2}
              />
            ) : (
              <>
                {/* Player */}
                ...
                >
                  ...
                  >
                    ...
                    >
                      {playing ? (
                        ...
                        <PlayIcon size={24} style={{ marginLeft: 3 }} />
                      )}
                    </button>
                    <div style={{ flex: 1 }}>
                      <input
                  ...
                  <audio
                    ...
                    onTimeUpdate={(e) => {
                      setPosMs(e.currentTarget.currentTime * 1000);
                    }}
                    onEnded={() => {
                      setPlaying(false);
                    }}
                ...
                >
                  ...
                  <div>
                    ...
                    {podcast.articles.map((a) => {
                      const active =
                        posMs >= a.startTimeMs && posMs < a.endTimeMs;
                      return (
                        <a
                          key={a.fileId}
                          href={`/article/${a.fileId}`}
                          onClick={navClick(() => {
                            router.push('article/:id', { id: a.fileId });
                          })}
                          className="pp-art"
                          style={
                            active
                              ? { background: 'rgba(214,38,29,0.06)' }
                              : undefined
                          }
                          data-id="a"
                        >

    ...
    <div style={pageStyle}>
      ...
      <div style={{ maxWidth: 1040, margin: '0 auto', padding: 'clamp(18px,4vw,40px)' }}>
        ...
        <header style={{ textAlign: 'center', padding: '14px 0 6px' }}>
          ...
          <div style={{ fontFamily: 'Spectral, serif', fontStyle: 'italic', fontSize: 20, color: '#2c2722', marginTop: 10 }}>
            Stories one side is barely covering. The gap is the story.
          </div>
        </header>

        {items === null && <p style={{ ...mono, color: C.muted, textAlign: 'center', marginTop: 30 }}>Finding the gaps…</p>}
        {items !== null && items.length === 0 && (
          <p style={{ fontFamily: 'Spectral, serif', fontSize: 19, textAlign: 'center', marginTop: 30 }}>
            No blindspots right now — coverage is unusually even. Check back after the next sweep.
          </p>
        )}

        {items !== null && items.length > 0 && (
          <section className="bs-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 22, marginTop: 26 }}>
            {items.map((c) => (
              <article key={c.id} style={{ border: `3px double ${C.ink}`, padding: '20px 22px', display: 'flex', flexDirection: 'column' }}>
                {c.blindspotSide && <div style={{ marginBottom: 12 }}><BlindspotBadge side={c.blindspotSide} /></div>}
                <h3 style={{ fontFamily: 'Spectral, serif', fontWeight: 600, fontSize: 24, lineHeight: 1.1, margin: '0 0 12px' }}>{c.title}</h3>
                {c.summary && <p style={{ fontFamily: 'Spectral, serif', fontSize: 16, lineHeight: 1.5, color: '#2c2722', margin: '0 0 16px' }}>{c.summary}</p>}
                <BiasBar b={c.biasBreakdown} height={14} />
                <div style={{ ...mono, fontSize: 11, letterSpacing: '.06em', textTransform: 'uppercase', color: C.muted, marginTop: 10 }}>

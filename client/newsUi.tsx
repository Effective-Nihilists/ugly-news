...
export interface ClusterCard {
  ...
  blindspotSide: BiasBucket | null;
  factualityAvg: number | null;
  articleCount: number;
  sourceCount: number;
...
export interface ClusterCoverageItem {
  ...
  sourceName: string;
  bucket: BiasBucket | null;
  factuality: string | null;
  uri: string | null;
  articleCount: number;
}
export interface UglyTake {
  id: string;
  title: string;
  markdown: string;
...
export interface ClusterFull extends ClusterCard {
  ...
  uglyTake: UglyTake | null;
}
export interface UglyTakeCard {
  clusterId: string;
  category: string;
  satireTitle: string;
  satireImageUri: string | null;
  satireSnippet: string;
  lastUpdatedAt: number;
}
    ...
    >
      ...
      {lead && (
        <LeadCluster
          c={lead}
          onOpen={() => {
            router.push('story/:id', { id: lead.id });
          }}
        />
      )}
      <div
      ...
      >
        {rest.map((c) => (
          ...
          >
            <div style={{ ...kicker, color: C.accent, marginBottom: 6 }}>
              {c.category}
            </div>
            <h3
              style={{
            ...
            >
              <span>
                {c.sourceCount} sources · {factualityLabel(c.factualityAvg)}
              </span>
  ...
  return (
    ...
    >
      <div
        style={{
          minHeight: 280,
          borderRight: `3px double ${C.ink}`,
          background: c.topImageUri
            ? `center/cover no-repeat url(${c.topImageUri})`
            : `repeating-linear-gradient(45deg, rgba(26,23,20,0.10) 0 2px, transparent 2px 9px), ${C.paper2}`,
        }}
      />
      <div
        style={{
      ...
      >
        <div style={kicker}>
          {c.category} · {c.sourceCount} sources
        </div>
        <h2
          style={{
            fontFamily: 'Spectral, serif',
    ...
    >
      ...
      <div style={{ border: `3px double ${C.ink}`, margin: '34px 0' }}>
        ...
        >
          ...
          <span style={{ ...kicker, color: C.paper, opacity: 0.85 }}>
            what one side isn't telling you
          </span>
        </div>
        {items.map((c, i) => (
          <a
            ...
            data-id="blindspot-row"
            className="bs-row"
            style={{
              borderBottom:
                i < items.length - 1 ? '1px solid rgba(26,23,20,0.25)' : 'none',
              textDecoration: 'none',
              color: C.ink,
            }}
          >
            ...
            <span className="bs-stats" style={{ ...kicker, fontSize: 12 }}>
              {c.biasBreakdown.left}L · {c.biasBreakdown.center}C ·{' '}
              {c.biasBreakdown.right}R →
            </span>
          </a>
        ))}
        <a
          href="/blindspot"
          onClick={navClick(() => {
            router.push('blindspot', {});
          })}
          data-id="see-all-blindspots"
          style={{
            display: 'block',
            textAlign: 'center',
            padding: '12px 18px',
            ...
            color: C.accent,
            textDecoration: 'none',
            borderTop: '1px solid rgba(26,23,20,0.25)',
          }}
        >
          See all blindspots →
        </a>
        ...
        >
          {nav.map((n) => (
            <a
              key={n.key}
              href={n.href}
              onClick={navClick(n.go)}
              data-id={`press-nav-${n.key}`}
              style={{
                fontFamily: 'IBM Plex Mono, monospace',
                fontSize: 11.5,
                letterSpacing: '.12em',
...
export function UglyTakeTile({
  ...
  return (
    <a
      href={`/ugly-take/${take.clusterId}`}
      onClick={navClick(onOpen)}
      data-id="ugly-take-tile"
      style={{
        textDecoration: 'none',
        color: C.ink,
        border: `3px double ${C.ink}`,
    ...
    >
      {take.satireImageUri && (
        <div
          style={{
            height: 128,
            background: `center/cover no-repeat url(${take.satireImageUri})`,
...
export function UglyTakeSection({
  takes,
}: {
  takes: UglyTakeCard[];
    ...
    >
      ...
      >
        ...
        <a
          ...
          style={{ ...kicker, color: C.accent, textDecoration: 'none' }}
          data-id="the-satire-desk"
        >
          the satire desk →
        </a>
      </div>
      {lead && (
        <a
          href={`/ugly-take/${lead.clusterId}`}
          onClick={navClick(() => {
            open(lead.clusterId);
          })}
          data-id="ugly-take-feature"
          ...
          >
            <div
              style={{
                minHeight: 300,
                borderRight: `3px double ${C.ink}`,
                background: lead.satireImageUri
                  ? `center/cover no-repeat url(${lead.satireImageUri})`
                  : `repeating-linear-gradient(-45deg, rgba(26,23,20,0.10) 0 2px, transparent 2px 9px), ${C.paper}`,
              }}
            />
            <div
              style={{

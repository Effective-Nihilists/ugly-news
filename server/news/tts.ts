...
export function preprocessTextForTTS(
  ...
  if (emotionHint && emotionHint !== 'neutral') {
    const hintToMarkup: Record<
      Exclude<PodcastSpeakerEmotion, 'neutral'>,
      InWorldEmotionMarkup
    > = {
      happy: '[happy]',
      sad: '[sad]',
      angry: '[angry]',
    ...
    if (/(hilarious|funny|joke|ridiculous|absurd)/.test(t))
      emotionMarkup = '[laughing]';
    else if (/(secret|whisper|rumor|allegedly|sources say)/.test(t))
      emotionMarkup = '[whispering]';
    else if (/(outrage|furious|scandal|slams|attacks)/.test(t))
      emotionMarkup = '[angry]';
    else if (/(tragic|devastating|heartbreaking|loss|passed away)/.test(t))
      emotionMarkup = '[sad]';
    else if (/(shocking|breaking|unbelievable|incredible|just in)/.test(t))
      emotionMarkup = '[surprised]';
    else if (/(terrifying|scary|horror|danger)/.test(t))
      emotionMarkup = '[fearful]';
    else if (/(disgusting|gross|disturbing|vile)/.test(t))
      emotionMarkup = '[disgusted]';
    else if (/(exciting|amazing|wonderful|celebrate|congratulations)/.test(t))
      emotionMarkup = '[happy]';
  }
  ...
  if (processed.length > 100 && Math.random() > 0.7) {
    ...
    processed =
      fillers[Math.floor(Math.random() * fillers.length)]! + processed;
  }
  ...
  if (nonVerbalCue === 'laugh' || nonVerbalCue === 'chuckle')
    processed = `${processed} [laughing]`;
  return processed;
}
...
export async function generateSegmentTTS(
  ...
  const temperature = emotionHint
    ? EMOTION_TEMPERATURES[emotionHint]
  ...
  let rendered;
  try {
    rendered = await renderSegmentCached({
      text: processedText,
      voiceId,
      temperature,
    });
    ...
    timestampInfo: words.length
      ? {
          wordAlignment: {
            ...
            wordEndTimeSeconds: words.map(
              (w) => (w.startMs + w.durationMs) / 1000,
            ),

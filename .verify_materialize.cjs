const {
  neon,
} = require('/Users/admin/Documents/GitHub/ugly-news/node_modules/@neondatabase/serverless');
const url =
  require('/Users/admin/.ugly-studio/projects/ad0eu8yee9/publish-state.json')
...
(async () => {
  for (let i = 0; i < 40; i++) {
    ...
    if (r.n >= 5) {
      console.log(
        `[${now}] RESULT: post-20:00 articles=${r.n}, COLUMN populated=${r.col}, data.embedding blob remaining=${r.blob}`,
      );
      console.log(
        r.col > 0 && r.col >= r.n - 1
      ...
      const s =
        await sql`select created, (embedding is not null) col, jsonb_typeof(data->'embedding') blob, data->>'title' t
        from "file" where data->>'type'='markdown' and created > ${MARK} order by created desc limit 3`;
      for (const x of s)
        console.log(
          '   ',
          x.created.toISOString().slice(11, 19),
          'COLUMN=' + x.col,
          'blob=' + x.blob,
          '|',
          (x.t || '').slice(0, 40),
